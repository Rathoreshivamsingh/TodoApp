...
async def read_all_by_user(request: Request,page: int = 1, page_size: int = 5,db: Session = Depends(get_db)):
    ...
    todos = db.query(models.Todos) \
        ...
        .limit(page_size) \
        .all()
    total_todos = db.query(models.Todos).filter(models.Todos.owner_id == user.get("id")).count()
    total_pages = (total_todos + page_size - 1) // page_size 

    return templates.TemplateResponse("home.html", {"request": request, "todos": todos, "user": user, "page": page, "page_size":page_size,
